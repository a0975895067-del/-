...
assert.match(route, /failures >= 5 \? new Date\(Date\.now\(\) \+ 15 \* 60_000\)/, '密碼錯誤五次須鎖定15分鐘');
assert.match(route, /counter == null[\s\S]+?failed_attempts[\s\S]+?15 \* 60_000/, '開發者動態碼錯誤也須累計並鎖定');
assert.match(route, /direct-application-ip:[\s\S]+?5, 15 \* 60[\s\S]+?direct-application-email:[\s\S]+?3, 15 \* 60/, '重複申請限制須在15分鐘後解除');
assert.match(route, /latestAllowedReset[\s\S]+?UPDATE rate_limits SET reset_at/, '舊的較長鎖定紀錄須縮短為目前設定的15分鐘');
assert.match(route, /重複登入錯誤，帳號已暫停登入；請於 15 分鐘後再登入。/, '鎖定訊息須明確告知15分鐘');
