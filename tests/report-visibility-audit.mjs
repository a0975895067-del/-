...
assert.match(backend, /authenticate\(request, \['student', 'teacher', 'developer', 'approved_user'\]\)/, 'Every authenticated learner or tester role must be able to save reports.');
...
assert.match(report, /\['student',\s*'teacher',\s*'developer',\s*'approved_user'\]\.includes\(role\)/, 'Client must upload reports for students and authenticated tester roles.');
